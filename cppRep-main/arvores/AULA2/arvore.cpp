...
} NO, *PONT; // no = struct,, *pont ponteiro p struct

/* Cria um novo (aloca memoria e preenche valores) no com chave=ch e retorna
   seu endereco. */
PONT criarNovoNo(TIPOCHAVE ch)
{
  // PONT novoNo = (PONT)malloc(sizeof(NO)); //(PONT) -> cast , transformar o tipo da variavel
  NO *novoNo = new (NO);
  novoNo->esq = NULL;
  novoNo->dir = NULL;
  novoNo->chave = ch;
...
int main(int argc, char **argv)
{
  ...
  cout << "INSERINDO 20 <<" << endl;
  inserirAVL(&arvoreAVL, 20, mudou);
  cout << "INSERINDO 15 <<" << endl;
  inserirAVL(&arvoreAVL, 15, mudou);
  cout << "INSERINDO 23 <<" << endl;
  inserirAVL(&arvoreAVL, 23, mudou);

  cin >> n;

  exibirArvore(arvoreAVL);
  fflush(stdout);
